...
ItÄôs built from scratch using advanced Excel formulas, structured references, dynamic arrays, and user-driven controls, and is designed to feel like a real-world analytics tool that leadership could use to plan, forecast, and present to executives.
...
- Allow users to adjust **operational and financial levers** to simulate different Äúwhat-ifÄù scenarios.
...
| Sheet | Purpose |
| --- | --- |
| `INPUT_DATA` | Contains all baseline monthly data (Jan 2023ÄìSept 2025). |
...
- **Forecasted Volume** Üí accounts for volume growth and compounding over time
- **Forecasted Cost/Patient** Üí inflation-adjusted using user inputs
- **Forecasted Readmission Rate** Üí reduced based on target efficiency
- **Forecasted Reimbursement %** Üí adjusted by payer mix assumptions
- **Forecasted Revenue, Cost, Margin, and Savings** Üí fully automated through SUMIFS and structured table logic
...
The dashboard displays both **Department-level KPIs** (based on the userÄôs dropdown selection) and **Global Hospital KPIs** for an overall view.
...
A horizontal bar chart visualizes each departmentÄôs profitability, helping identify the weakest-performing areas.
...
A ÄúPerformance AlertsÄù section automatically updates with contextual warnings, such as:

> Department operating at a loss. Review cost drivers.
> 

This alert is condition-based, triggering when a departmentÄôs forecasted margin is below 0%.
...
ItÄôs something that could realistically help a hospitalÄôs finance or operations department run Äúwhat-ifÄù scenarios, forecast costs, and present performance to executives.
...
Building this model pushed me to apply nearly every Excel concept IÄôve learned, from forecasting functions and dynamic ranges to professional dashboard design and data visualization.
